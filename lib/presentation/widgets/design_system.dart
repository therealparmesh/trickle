...
final class CommandTile extends StatelessWidget {
  const CommandTile({
    required this.icon,
    required this.label,
    required this.onTap,
  ...
  Widget build(BuildContext context) {
    ...
    return Semantics(
      ...
      label: [
        label,
        if (badge case final count?) '$count items',
        ?detail,
      ].join('. '),
      ...
      child: SignalPanel(
        onTap: onTap,
        accent: color,
        color: AppConstants.elevated.withValues(alpha: 0.74),
        padding: EdgeInsets.fromLTRB(13, textScale > 1.5 ? 14 : 11, 11, 11),
        child: ConstrainedBox(
          constraints: const BoxConstraints(minHeight: 50),
          child: Row(
            children: [
              SignalIcon(icon: icon, color: color, size: 42),
