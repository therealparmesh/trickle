...
- Candidate release: `1.2.4` (`versionCode` 56)
...
Build 56 can be the first Android store release if live Play and Zapstore checks confirm that neither has consumed it. Before building, query every available track and the Zapstore listing; repository history alone is not proof.
...
Record pass/fail evidence in the private release archive. Any code, dependency, manifest, resource, or runtime configuration change invalidates both candidate artifacts. Rebuild both from the new commit; retain version code 56 only if neither store has consumed it, otherwise increment the version code.
...
7. Run `bundle exec fastlane android play_status` and confirm production has no version 56 before promotion.
...
CONFIRM_PRODUCTION=1 bundle exec fastlane android play_promote \
  version_code:56 rollout:0.1
```

The lane must reject a rollout outside `0 < rollout < 1`; completion to 100 percent is a separate deliberate operation after vitals review. Build 56's first production transition remains a Play Console gate because the API cannot publish a draft app.

Do not reuse a rejected version code. If Play consumes 56 and requires another binary, increment `pubspec.yaml`, commit `chore: prepare 1.2.4 build 57`, rebuild both artifacts, and publish the matching version to both stores.

## Phase 8: Zapstore rollout

1. Create Git tag `v1.2.4-android.56` at the exact release commit.
