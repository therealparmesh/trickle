...
5. From Home, tap Add Feed under Feeds to enter a public RSS, Atom, JSON Feed, website URL, or Nostr `npub`/`nprofile`. Nostr profile feeds show verified root posts and omit replies and reposts. Posts may include Markdown, content warnings, images, native audio, or direct video. Open Feed settings to move a non-podcast source to a previous or new optional category. Reader > Feeds can rename a category for all matching sources.
...
9. Settings contains global playback speed, download cleanup, standard OPML import, separate OPML exports for podcasts, feeds (RSS and YouTube), and all compatible subscriptions, plus local backup controls. OPML folders preserve reader categories. The local backup also preserves Nostr profiles and their portable state.
