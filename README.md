...
The visual system uses clipped control geometry, functional state rails, and a sparse signal-line backdrop instead of decorating every content row. Cyan identifies listening actions, magenta identifies feed actions, and acid green is reserved for active playback. Content lists remain continuous and low-chrome. Brief stepped signal-lock motion marks startup and pushed collection or detail entrances without drawing scan bars over content; playback, Picture in Picture, article reading, and in-place controls remain stable. System Reduce Motion disables the effect. Display typography is limited to page and section hierarchy; reading and metadata use the more neutral text face. Controls reflow at accessibility text sizes rather than shrinking labels or touch targets.
